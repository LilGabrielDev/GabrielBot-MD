...
function initConfig() {
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(
      configPath,
      JSON.stringify({ enabled: false, mode: "all" }, null, 2)
    );
  }
...
async function autoreadCommand(sock, chatId, message) {
  try {
    ...
    if (args.length > 0) {
      ...
      } else if (action === "group" || action === "gc") {
        config.enabled = true;
        config.mode = "group";
      } else if (action === "private" || action === "pc") {
        config.enabled = true;
        config.mode = "private";
      } else {
        await sock.sendMessage(chatId, {
          text: `❌ Invalid option!\n\n*Usage:*\n- ${getPrimaryPrefix()}autoread on/off\n- ${getPrimaryPrefix()}autoread group (Group only)\n- ${getPrimaryPrefix()}autoread private (Private only)`,
          contextInfo: {
            forwardingScore: 1,
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
    ...
    let statusMsg = `✅ Auto-read has been ${
...
async function handleAutoread(sock, message) {
  ...
  if (config.enabled) {
    ...
    if (config.mode === "private" && isGroup) return false;

...
function isBlocked(userId) {
    try {
        const blockedUsers = JSON.parse(fs.readFileSync('./data/blocked.json', 'utf8').replace(/^\uFEFF/, ""));
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!targets.length) {
            ...
            return reply(fmt(
                ...
                (isPrivate ? `• Just use \`.${verb}\` in private chat to ${verb} the current user` : '')
            ));
        }

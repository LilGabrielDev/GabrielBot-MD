...
function buildContextPrompt(jid, currentQuery) {
    ...
    const history = mem.map(m => `${m.role === 'user' ? 'User' : 'Gabriel'}: ${m.text}`).join('\n');
    return `You are Gabriel, a smart, friendly WhatsApp AI assistant. Stay in character. Be concise and helpful.\n\nConversation so far:\n${history}\n\nUser: ${currentQuery}\nGabriel:`;
}
const os = require('os');
...
const BOT_IDENTITY = {
    ...
    language: 'Node.js',
    library: 'Baileys',
    repo: 'https://github.com/LilGabrielDev/GabrielBot-MD',
    get platform() { return detectPlatform(); },
    developer: 'GabrielDev',
    ownerName: config.OWNER_NAME || 'Gabriel MD',
...
const intentMap = [
    ...
    // NOTE: \bgithub\b only fires for plain user profiles (not repo paths / GabrielDev)
    ...
    // ── GitHub GabrielDev ────────────────────────────────────────────────────
    { pattern: /\bGabrielDev\b.*\b(repos?|github|projects?)\b|\b(LilGabrielDev|LilGabrielDevrepos)\b/i, cmd: 'LilGabrielDev', label: '🐙 Fetching LilGabrielDev repos',  strip: /\bGabrielDev\b|\bGabrielrepos?\b/gi },
...
const smartResponses = [
    { p: /^(hi+|hello+|hey+|howdy|sup|yo+|hii+|ello)\b/i,
      r: [`Hey! 👋 What can I do for you today?`, `Hello! 😊 How can I help you?`, `Hey there! I'm Gabriel, your WhatsApp assistant. What do you need? 🤖`] },
    ...
    { p: /what('?s| is) your name|your name\b|who are you\b/i,
      r: [`I'm *Gabriel* 🤖 — your intelligent WhatsApp assistant! Built on ${BOT_IDENTITY.name} v${BOT_IDENTITY.version}.`] },
    { p: /what can you do|your (capabilities|powers|features|abilities)\b/i,
      r: [`I can: 🎵 play music, 📸 make stickers, ⬇️ download from TikTok/Instagram/YouTube, 🌤️ check weather, 📚 search Wikipedia, 🌐 translate text, 👥 manage groups, and 1400+ commands! Type \`Gabriel help\` to see everything.`] },
    ...
    { p: /help\b/i,
      r: [`Type \`Gabriel\` (no prefix needed) to see everything I can do! Or try:\n• \`Gabriel play <song name>\`\n• \`Gabriel weather <city>\`\n• \`Gabriel wiki <topic>\`\n• \`Gabriel sticker\` (reply to a photo)`] },
];
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            const res = await axios.post(
                ...
                { message: prompt },
                {
                    headers: { 'Content-Type': 'application/json', 'User-Agent': 'GabrielMD-Bot/2.0' },
                    timeout: 12000,
                }
            );
...
function offlineSmartReply(query, pluginKeys) {
    ...
    const catchAll = [
        ...
        `Gabriel here! 💬 My response engine is warming up. One more try should do it — or use \`.menu\` to browse all commands!`,
    ];
...
async function askFreeAI(query, jid, systemPrompt, _pluginKeys) {
    ...
    const popcatPromise = axios.get(
        ...
        '&owner=' + encodeURIComponent(config.OWNER_NAME || 'Gabriel') + '&botname=Gabriel',
        { timeout: 7000 }
...
const agentActions = {
    ...
    about_bot:      /about\s*(the\s*)?(bot|Gabriel|yourself)|who\s*are\s*you|what\s*are\s*you|tell\s*me\s*about\s*(yourself|Gabriel|this\s*bot)/i,
    about_platform: /platform|server|hosting|where\s*(are\s*you|is\s*(the\s*bot|Gabriel))\s*(running|hosted)|system\s*info|server\s*info|specs/i,
    about_owner:    /who\s*(is\s*)?(the\s*)?(owner|creator|developer|made|built|coded)|your\s*(owner|creator|dev)/i,
    features:       /features|what\s*can\s*(you|the\s*bot|Gabriel)\s*do|capabilities|abilities|powers/i,
    ...
    // ── GitHub GabrielDev (read-only) ─────────────────────────────────────────
    github_GabrielDev: /\b(GabrielDev|GabrielDevwb|GabrielDev|Gabrielrepos)\b|\bGabrielDev\s+(repos?|github|projects?|code|files?|zip|download)\b/i,

    // ── GitHub repo file/zip fetch ("send me a clip of GabrielDev/repo") ──────
    github_repo_zip:  /\b(clip|zip|archive|download|send\s+me)\b.*\b([\w-]+\/[\w-]+)\b|\b([\w-]+\/[\w-]+)\b.*\b(zip|clone|download)\b/i,
    github_repo_file: /\b(get|fetch|read|show|send)\b.+\b(from|in|of)\b.+\b([\w\/-]+\/[\w\/-]+\.[\w]+)\b|\b(readme|package\.json|index\.js|config|\.env)\b.*\b(from|in|of)\b.+\bGabriel/i,
...
module.exports = {
    commands: ['Gabriel', 'agent', 'do', 'assistant', 'ask'],
    description: 'Gabriel - AI assistant that runs commands, creates content, searches the web, and knows everything about the bot',
    permission: 'public',
    run: async (sock, message, args, ctx) => {
        ...
        if (!query) return reply(
            ...
            `🎵 *Music & Media*\n` +
            `• "Gabriel play <song>" • "Gabriel lyrics <song>"\n` +
            `• "Gabriel tiktok <url>" • "Gabriel youtube video <name>"\n` +
            `• "Gabriel speak Hello world" _(text-to-speech)_\n\n` +
            `🎨 *AI & Images*\n` +
            `• "Gabriel generate image of a lion in space"\n` +
            `• "Gabriel describe" _(reply to any photo)_\n` +
            `• "Gabriel summarize" _(reply to a long message)_\n` +
            `• "Gabriel quotly" _(reply to a message → quote sticker)_\n\n` +
            `🧩 *Fun & Knowledge*\n` +
            `• "Gabriel riddle" — get a brain teaser\n` +
            `• "Gabriel proverb" / "Gabriel wisdom" — wise saying\n` +
            `• "Gabriel rhyme moon" — words that rhyme\n\n` +
            `🐙 *GitHub*\n` +
            `• "Gabriel GabrielDev repos" — list GabrielDev repos\n` +
            `• ".GabrielDev <repo>" — repo stats & files\n` +
            `• ".GabrielDev zip <repo>" — download link\n\n` +
            `📲 *Downloads*\n` +
            `• "Gabriel instagram <url>" • "Gabriel facebook <url>"\n` +
            `• "Gabriel spotify <name>" • "Gabriel pinterest <query>"\n\n` +
            `⏰ *Productivity*\n` +
            `• "Gabriel remind me in 30m to call mom"\n` +
            `• "Gabriel save note shopping: milk, eggs"\n` +
            `• "Gabriel get my notes"\n` +
            `• "Gabriel create a poll: Question | A | B | C"\n` +
            `• "Gabriel schedule message at 9pm: team meeting"\n\n` +
            `🛠️ *Tools*\n` +
            `• "Gabriel sticker" _(reply to photo)_\n` +
            `• "Gabriel translate hello to french"\n` +
            `• "Gabriel wiki artificial intelligence"\n` +
            `• "Gabriel weather Nairobi"\n` +
            `• "Gabriel qr code https://example.com"\n\n` +
            `👥 *Group Management*\n` +
            `• "Gabriel change group name to X"\n` +
            `• "Gabriel mute/unmute group"\n` +
            `• "Gabriel tag all members"\n` +
            `• "Gabriel kick @user" • "Gabriel promote @user"\n` +
            `• "Gabriel create a poll: Vote | Yes | No"\n\n` +
            `✍️ *Content Creation*\n` +
            `• "Gabriel create a bio / welcome / rules / poem"\n` +
            `• "Gabriel write an email about X"\n\n` +
            `🌐 *Info & Web*\n` +
            `• "Gabriel weather / news / search / ip info"\n` +
            `• "Gabriel about bot / platform / owner / settings"\n\n` +
            `🧠 *AI Chat* — Ask me anything! I remember our conversation.\n` +
            `• "Gabriel forget" — clears chat memory\n\n` +
            `📋 *Run Any Command:* "Gabriel run <command>"\n\n` +
        ...
        if (agentActions.about_bot.test(query)) {
            ...
            response =
                `👑 *Bot Owner*\n\n` +
                `• Name: *${config.OWNER_NAME || 'Gabriel MD'}*\n` +
                `• Number: *+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}*\n` +
                `• Bot: *${config.BOT_NAME || 'Gabriel MD'}*\n` +
            ...
            const quotes = [
                ...
                { text: `${topic || 'Devnology'} is not just a tool, it's a mindset.`, author: config.OWNER_NAME },
            ...
            const topic = query.replace(agentActions.create_poem, '').trim() || 'Devnology';
            response =
                `✍️ *Poem: ${topic}*\n\n` +
            ...
            try {
                const res = await axios.get('https://saurav.Dev/NewsAPI/top-headlines/category/Devnology/us.json', { timeout: 8000 });
                const articles = res.data?.articles?.slice(0, 5) || [];
                if (articles.length) {
                    const newsText = articles.map((a, i) => `*${i + 1}.* ${a.title}\n   _${a.source?.name || 'Unknown'}_`).join('\n\n');
                    response = `📰 *Latest Dev News*\n\n${newsText}`;
                } else {
            ...
            response =
                ...
                `👥 *Group Management*\n` +
                `• "Gabriel change group name to X"\n` +
                `• "Gabriel set group description to X"\n` +
                `• "Gabriel mute/unmute group"\n` +
                `• "Gabriel lock/unlock group"\n` +
                `• "Gabriel get group link"\n` +
                `• "Gabriel tag all members"\n` +
                `• "Gabriel list admins"\n` +
                `• "Gabriel kick/add/promote/demote"\n\n` +
                `🌐 *Web Access*\n` +
        ...
        // of how the message was triggered (no rawCmd detection issues).
            ...
            if (!newDesc) return reply(`❓ What should the group description say?\n\nExample: _Gabriel set group description to Welcome to our group!_`);
            ...
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _Gabriel kick_\n\nOr tag them: _Gabriel kick @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _Gabriel promote_\n\nOr tag them: _Gabriel promote @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _Gabriel demote_\n\nOr tag them: _Gabriel demote @member_`);
            try {
        ...
        // ── Productivity handlers ─────────────────────────────────────────────
            ...
            // "Gabriel remind me in 30 minutes to check email"
            const pm = pluginMap();
            ...
            if (!noteContent) return reply(`❓ What should I save?\n\nExample: _Gabriel save note shopping list: milk, eggs, bread_`);
            if (plugin) {
            ...
            // Parse "Gabriel create a poll: Question | Option A | Option B | Option C"
            const pollContent = query.replace(agentActions.poll, '').replace(/^[:\-\s]+/, '').trim();
            if (!pollContent) {
                return reply(
                    `📊 *Create a Poll*\n\n` +
                    `Format: _Gabriel create a poll: Question | Option 1 | Option 2 | Option 3_\n\n` +
                    `Example: _Gabriel create a poll: Favorite color? | Red | Blue | Green_`
                );
            }
            if (plugin) {
        ...
        // ── Media / AI handlers ───────────────────────────────────────────────
            ...
            const prompt = query
                ...
                .trim();
            if (!prompt) return reply(`🎨 What image should I generate?\n\nExample: _Gabriel generate an image of a lion wearing a gold crown in a forest_`);
        ...
        // ── GitHub GabrielDev (read-only) ─────────────────────────────────────
        } else if (agentActions.github_GabrielDev.test(query)) {
            const pm = pluginMap();
            const ghPlugin = pm.get('GabrielDev') || pm.get('Gabrielrepos');
            // Pass the cleaned query as args so the plugin can parse subcommands
            const cleanedArgs = query
                .replace(/\b(GabrielDev|GabrielDevwb|GabrielDev|Gabrielrepos)\b/gi, '')
        ...
        // ── GitHub repo ZIP download ("send me a clip of GabrielDev/Gabriel-md-bot") ──
            ...
            if (!fullRepo || fullRepo.includes('Gabriel') === false && !fullRepo.includes('/')) {
                return reply(`📦 Please specify the repo in *owner/repo* format.\n\nExample: _Gabriel send me a clip of GabrielDev/Gabriel-md-bot_`);
            }
            const [owner, repo] = fullRepo.split('/');
            await safeSend({ text: `📦 _Preparing download link for *${fullRepo}*..._` }, { quoted: message });
            try {
                const ghHeaders = { 'User-Agent': 'GabrielMD-Bot/2.0', 'Accept': 'application/vnd.github+json' };
                ...
                reply(
                    ...
                    `*🔗 View on GitHub*\n${r.html_url}\n\n` +
                    `_Use \`.GabrielDev ${repo} <filepath>\` to read specific files._`
                );
            } catch (err) {
                const branch = 'main';
                reply(
        ...
        // ── GitHub repo file fetch ────────────────────────────────────────────
            ...
            const repoMatch = query.match(/\b([\w-]+\/[\w-]+)\b/);
            const fileKeyword = query.match(/\b(readme|package\.json|index\.js|config|handler|Gabriel)\b/i)?.[1] || '';

            if (!repoMatch) {
                return reply(`📄 Specify the repo and file.\n\nExamples:\n• _gabriel get README from LilGabrielBot/gabriel-md-bot_\n• _.LilGaberielDev gabriel-md-bot README.md_`);
            }
            ...
            try {
                const ghHeaders = { 'User-Agent': 'GabrielMD-Bot/1.0', 'Accept': 'application/vnd.github+json' };
                ...
                if (err.response?.status === 404) {
                    reply(`❌ File not found: \`${owner}/${repo}/${filePath}\`\n\n_Use \`.lilgabrieldev ${repo}\` to browse available files._`);
                } else {
        ...
        // ── Rhyme dispatch ────────────────────────────────────────────────────
            ...
            if (word) {
                ...
                reply(`🎵 *Rhyme Finder*\n\nExample: _gabriel what rhymes with moon_\n\nOr use: \`.rhyme <word>\``);
            }
            return;
            ...
            if (smart) {
                ...
                const systemPrompt =
                    `You are Gabriel, a smart WhatsApp bot assistant. ` +
                    `Owner: ${config.OWNER_NAME || 'Gabriel'}. Bot: ${config.BOT_NAME || 'Gabriel MD'}.\n` +
                    ...
                    try {
                        ...
                        if (apiKey) {
                            ...
                            const geminiHistory = [
                                ...
                                { role: 'model', parts: [{ text: `Got it! I'm Gabriel, ready to help. 🤖` }] },
                                ...mem.slice(0, -1).map(m => ({
                ...
                response = aiReply.startsWith('🤖 *Gabriel*') ? aiReply : `🤖 *Gabriel*\n\n${aiReply}`;

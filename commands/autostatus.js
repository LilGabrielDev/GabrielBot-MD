...
async function autoStatusCommand(sock, chatId, msg, args) {
  try {
    ...
    if (!args || args.length === 0) {
      ...
      await sock.sendMessage(chatId, {
        text: `🔄 *Auto Status Settings*\n\n📱 *Auto Status View:* ${status}\n💫 *Status Reactions:* ${reactStatus}\n\n*Commands:*\n${getPrimaryPrefix()}autostatus on - Enable auto status view\n${getPrimaryPrefix()}autostatus off - Disable auto status view\n${getPrimaryPrefix()}autostatus react on - Enable status reactions\n${getPrimaryPrefix()}autostatus react off - Disable status reactions`,
        ...channelInfo,
      });
      return;
    }
    ...
    if (command === "on") {
      ...
      if (!args[1]) {
        await sock.sendMessage(chatId, {
          text: `❌ Please specify on/off for reactions!\nUse: ${getPrimaryPrefix()}autostatus react on/off`,
          ...channelInfo,
        });
        return;
      }
      ...
      if (reactCommand === "on") {
        ...
        await sock.sendMessage(chatId, {
          text: `❌ Invalid reaction command! Use: ${getPrimaryPrefix()}autostatus react on/off`,
          ...channelInfo,
        });
      }
    } else {
      await sock.sendMessage(chatId, {
        text: `❌ Invalid command! Use:\n${getPrimaryPrefix()}autostatus on/off - Enable/disable auto status view\n${getPrimaryPrefix()}autostatus react on/off - Enable/disable status reactions`,
        ...channelInfo,
      });
    }
  } catch (error) {

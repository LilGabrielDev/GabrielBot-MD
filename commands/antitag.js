...
async function handleAntitagCommand(
  sock,
  chatId,
  userMessage,
  senderId,
  ...
  try {
    ...
    switch (action) {
      case "on":
        ...
        if (existingConfig?.enabled) {
          await sock.sendMessage(
            chatId,
            { text: "⚠️ *_Antitag is already on_*" },
            { quoted: message },
          );
          return;
        }
        ...
        await sock.sendMessage(
          ...
          {
            text: result
              ? "*_Antitag has been turned ON_*"
              : "*_Failed to turn on Antitag_*",
          },
          { quoted: message },
        );
        break;
      ...
      case "off":
        ...
        await sock.sendMessage(
          chatId,
          { text: "🛑 *_Antitag has been turned OFF_*" },
          { quoted: message },
        );
        break;
      ...
      case "set":
        ...
        await sock.sendMessage(
          ...
          {
            text: setResult
              ? `*_Antitag action set to ${setAction}_*`
              : "*_Failed to set Antitag action_*",
          },
          { quoted: message },
        );
        break;
    ...
    await sock.sendMessage(
      chatId,
      { text: "❌ *_Error processing antitag command_*" },
      { quoted: message },
    );
  }
}
...
async function handleTagDetection(sock, chatId, message, senderId) {
  try {
    ...
    const allMentions = [
      ...new Set([...mentionedJids, ...textMentions, ...numericMentions]),
    ];

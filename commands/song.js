  ...
  try {
    ...
    if (!text) {
      await sock.sendMessage(
        chatId,
        { text: `ℹ️ Usage: ${getPrimaryPrefix()}song <song name or YouTube link>` },
        { quoted: message },
      );
      return;
    }
    ...
    for (const apiMethod of apiMethods) {
      try {
        ...
        if (!audioUrl) {
          console.log(
            `${apiMethod.name} returned no download URL, trying next API...`,
          );
        ...
        try {
          ...
          if (statusCode === 451) {
            console.log(
              `Download blocked (451) from ${apiMethod.name}, trying next API...`,
            );
            continue; // Try next API
          }
          ...
          try {
            ...
            const streamStatusCode =
              streamErr.response?.status || streamErr.status;
            if (streamStatusCode === 451) {
              console.log(
                `Stream download blocked (451) from ${apiMethod.name}, trying next API...`,
              );
            } else {
              console.log(
                `Stream download failed from ${apiMethod.name}:`,
                streamErr.message,
              );
            }
            continue; // Try next API
    ...
    if (!downloadSuccess || !audioBuffer) {
      throw new Error(
        "All download sources failed. The content may be unavailable or blocked in your region.",
      );
    }
    ...
    if (fileExtension !== "mp3") {
      try {
        ...
        throw new Error(
          `Failed to convert ${detectedFormat} to MP3: ${convErr.message}`,
        );
      }
    }
    ...
    if (err.message && err.message.includes("blocked")) {
      errorMessage =
        "❌ Download blocked. The content may be unavailable in your region or due to legal restrictions.";
    } else if (err.response?.status === 451 || err.status === 451) {
      errorMessage =
        "❌ Content unavailable (451). This may be due to legal restrictions or regional blocking.";
    } else if (
      err.message &&

...
async function setGroupDescription(sock, chatId, senderId, text, message) {
  ...
  if (!desc) {
    await sock.sendMessage(
      chatId,
      { text: `Usage: ${getPrimaryPrefix()}setgdesc <description>` },
      { quoted: message },
    );
    return;
  }
...
async function setGroupName(sock, chatId, senderId, text, message) {
  ...
  if (!name) {
    await sock.sendMessage(
      chatId,
      { text: `Usage: ${getPrimaryPrefix()}setgname <new name>` },
      { quoted: message },
    );
    return;
  }
...
async function setGroupPhoto(sock, chatId, senderId, message) {
  ...
  if (!imageMessage) {
    await sock.sendMessage(
      chatId,
      { text: `Reply to an image/sticker with ${getPrimaryPrefix()}setgpp` },
      { quoted: message },
    );
    return;
  }

...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  const tempInputBase = path.join(
    tmpDir,
    `igs_${Date.now()}_${Math.random().toString(36).slice(2)}`,
  );
  const tempInput = isAnimated
    ? `${tempInputBase}.mp4`
    : `${tempInputBase}.jpg`;
  const tempOutput = path.join(
    tmpDir,
    `igs_out_${Date.now()}_${Math.random().toString(36).slice(2)}.webp`,
  );
  ...
  if (isAnimated && webpBuffer.length > 1000 * 1024) {
    try {
      ...
      const tempOutput2 = path.join(
        tmpDir,
        `igs_out2_${Date.now()}_${Math.random().toString(36).slice(2)}.webp`,
      );
  ...
  const json = {
    ...
    "sticker-pack-name": settings.packname || "GabrielMDBot",
    emojis: ["📸"],
  };
  ...
  if (finalBuffer.length > 900 * 1024) {
    try {
      const tempOutput3 = path.join(
        tmpDir,
        `igs_out3_${Date.now()}_${Math.random().toString(36).slice(2)}.webp`,
      );
...
async function fetchBufferFromUrl(url) {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Both axios download attempts failed:",
        e1?.message || e1,
        e2?.message || e2,
      );
      throw e2;
...
async function forceMiniSticker(inputBuffer, isVideo, cropSquare) {
  ...
  const tempInput = path.join(
    tmpDir,
    `mini_${Date.now()}.${isVideo ? "mp4" : "jpg"}`,
  );
  ...
  const json = {
    "sticker-pack-id": crypto.randomBytes(32).toString("hex"),
    "sticker-pack-name": settings.packname || "Gabriel MD Bot",
    emojis: ["📸"],
  };
  const exifAttr = Buffer.from([
    0x49, 0x49, 0x2a, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01, 0x00, 0x41, 0x57,

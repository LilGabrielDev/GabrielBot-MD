...
async function setMentionCommand(sock, chatId, message, isOwner) {
  ...
  if (!qMsg)
    return sock.sendMessage(
      chatId,
        {
          text: "💬 Reply to a message or media (sticker/image/video/audio/document).",
        },
      { quoted: message },
    );
  ...
  if (qMsg.stickerMessage) {
    ...
    return sock.sendMessage(
      chatId,
      {
        text: "Unsupported. Reply to text/sticker/image/video/audio/document.",
      },
      { quoted: message },
    );
  ...
  return sock.sendMessage(
    chatId,
    { text: "Mention reply media updated." },
    { quoted: message },
  );
}

...
async function attpCommand(sock, chatId, message) {
  ...
  try {
    ...
    const webpPath = await writeExifVid(mp4Buffer, { packname: "Gabriel MD Bot" });
    ...
    await sock.sendMessage(
      chatId,
      { text: "❌ Failed to generate the sticker locally." },
      { quoted: message },
    );
  }
}
...
function renderTextToPngWithFfmpeg(text) {
  return new Promise((resolve, reject) => {
    ...
    ff.on("close", (code) => {
      ...
      reject(
        new Error(
          Buffer.concat(errors).toString() || `ffmpeg exited with code ${code}`,
        ),
      );
    });
  });

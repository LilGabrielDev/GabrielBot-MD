...
async function deleteCommand(sock, chatId, message, senderId) {
  try {
    const safeSenderId = senderId || message.key?.participant || message.key?.remoteJid || '';
    const { isSenderAdmin, isBotAdmin } = await isAdmin(sock, chatId, safeSenderId);
    ...
    if (!isSenderAdmin) {
      await sock.sendMessage(
        chatId,
        { text: `Only admins can use the ${getPrimaryPrefix()}delete command.` },
        { quoted: message },
      );
      return;
    }
    ...
    const mentioned =
      Array.isArray(ctxInfo.mentionedJid) && ctxInfo.mentionedJid.length > 0
    ...
    // If no number provided and not replying/mentioning, show usage message
      ...
      await sock.sendMessage(
        chatId,
        {
          text: `❌ Please specify the number of messages to delete.\n\nUsage:\n• ${getPrimaryPrefix()}del 5 - Delete last 5 messages from group\n• ${getPrimaryPrefix()}del 3 @user - Delete last 3 messages from @user\n• ${getPrimaryPrefix()}del 2 (reply to message) - Delete last 2 messages from replied user`,
        },
        { quoted: message },
      );
      return;
    ...
    if (toDelete.length === 0) {
      const errorMsg = deleteGroupMessages
        ? "ℹ️ No recent messages found in the group to delete."
        : "ℹ️ No recent messages found for the target user.";

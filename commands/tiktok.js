...
async function tiktokCommand(sock, chatId, message) {
  try {
    ...
    if (!isValidUrl) {
      return await sock.sendMessage(chatId, {
        text: "⚠️ That is not a valid TikTok link. Please provide a valid TikTok video link.",
      });
    }
        ...
        if (response.data && response.data.status) {
          ...
          if (response.data.data) {
            ...
            if (
              ...
              response.data.data.urls.length > 0
            ) {
              ...
              if (isVideo) {
                await sock.sendMessage(
                  chatId,
                  {
                    video: { url: mediaUrl },
                    mimetype: "video/mp4",
                    caption: "DOWNLOADED BY GABRIEL-BOT",
                  },
                  { quoted: message },
                );
              } else {
      ...
      if (videoUrl) {
        try {
          ...
          if (!isValidVideo && videoBuffer.length < 10000) {
            ...
            if (
              bufferText.includes("error") ||
              bufferText.includes("blocked") ||
              bufferText.includes("403")
            ) {

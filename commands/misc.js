    ...
    switch (sub) {
      ...
      case "its-so-stupid": {
        ...
        if (!dog) {
          await sock.sendMessage(
            chatId,
            { text: "ℹ️ Usage: .misc its-so-stupid <text>" },
            { quoted: message },
          );
          return;
        }
      ...
      case "namecard": {
        ...
        const [username, birthday, description] = joined
          .split("|")
          .map((s) => (s || "").trim());
        if (!username || !birthday) {
          await sock.sendMessage(
            chatId,
            {
              text: "ℹ️ Usage: .misc namecard username|birthday|description(optional)",
            },
            { quoted: message },
          );
          return;
      ...
      case "tweet": {
        ...
        const joined = rest.join(" ");
        const [displayname, username, comment, theme] = joined
          .split("|")
          .map((s) => (s || "").trim());
        if (!displayname || !username || !comment) {
          await sock.sendMessage(
            chatId,
            {
              text: "ℹ️ Usage: .misc tweet displayname|username|comment|theme(optional light/dark)",
            },
            { quoted: message },
          );
          return;
      ...
      case "youtube-comment": {
        ...
        const joined = rest.join(" ");
        const [username, comment] = joined
          .split("|")
          .map((s) => (s || "").trim());
        if (!username || !comment) {
          await sock.sendMessage(
            chatId,
            { text: "Usage: .misc youtube-comment username|comment" },
            { quoted: message },
          );
          return;
        }
      ...
      default:
        await sock.sendMessage(
          chatId,
          {
            text: "ℹ️ Usage: .misc <heart|horny|circle|lgbt|lesbian|nonbinary|pansexual|transgender|lied|lolice|simpcard|tonikawa|its-so-stupid <text>|namecard u|b|d?|nobitches <text>|oogway <q>|oogway2 <q>|tweet dn|un|c|theme?|youtube-comment un|c>",
          },
          { quoted: message },
        );
        break;

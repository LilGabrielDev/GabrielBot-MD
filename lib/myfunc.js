...
exports.getTime = (format, date) => {
  if (date) {
    return moment(date).tz("Africa/Nairobi").locale("en").format(format);
  } else {
    return moment.tz("Africa/Nairobi").locale("en").format(format);
  }
};
...
exports.jam = (numer, options = {}) => {
  ...
  let jam = moment(numer)
    .tz(options?.timeZone || "Africa/Nairobi")
    .format(format);
...
exports.smsg = (XeonBotInc, m, store) => {
  ...
  if (m.key) {
    ...
    if (m.isGroup)
      m.participant = XeonBotInc.decodeJid(m.key.participant) || "";
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender === (XeonBotInc.user && XeonBotInc.user.id);
      m.quoted.text =
        m.quoted.text ||
        m.quoted.caption ||
  ...
  m.reply = (text, chatId = m.chat, options = {}) =>
    Buffer.isBuffer(text)
      ? XeonBotInc.sendMedia(chatId, text, "file", "", m, {
          ...options,
        })

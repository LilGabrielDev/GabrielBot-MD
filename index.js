...
let phoneNumber = "254716427334";
let owner = JSON.parse(fs.readFileSync("./data/owner.json"));

global.botname = "Gabriel MD Bot";
global.themeemoji = "•";
  ...
  try {
    ...
    if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!global.phoneNumber) {
        ...
        phoneNumber = await question(
          chalk.bgBlack(
            chalk.greenBright(
              `Please type your WhatsApp number 😍\nFormat: 254714333524 (without + or spaces) : `,
            ),
          ),
        );
      }
      ...
      if (!pn("+" + phoneNumber).isValid()) {
        console.log(
          chalk.red(
            "Invalid phone number. Please enter your full international number (e.g., 15551234567 for US, 447911123456 for UK, etc.) without + or spaces.",
          ),
        );
        process.exit(1);
      }
    ...
    XeonBotInc.ev.on("connection.update", async (s) => {
      ...
      if (connection == "open") {
        ...
        console.log(
          chalk.yellow(
            `\n\n                  ${chalk.bold.blue(`[ ${global.botname || "Gabriel MD Bot"} ]`)}\n\n`,
          ),
        );

        const { isAutobioEnabled, startAutobioLoop } = require("./commands/autobio");
        if (isAutobioEnabled()) {
          startAutobioLoop(XeonBotInc);
          console.log("🔄 Auto Bio loop started");
        }
        console.log(
          chalk.cyan(`< ================================================== >`),
        );
        ...
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} GITHUB: LilGabrielDev`),
        );
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} WA NUMBER: ${owner}`),
        );
        console.log(
          chalk.magenta(`${global.themeemoji || "•"} CREDIT: Lil Gabriel Dev`),
        );
        console.log(
          chalk.green(
        ...
        for (const call of calls) {
          ...
          try {
            ...
            if (!antiCallNotified.has(callerJid)) {
              ...
              const messageText =
                state.mode === "reject"
                  ? "📵 Anticall is enabled. Your call was rejected."
          ...
          // Only block if mode is NOT 'reject'
